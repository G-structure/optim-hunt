...
def create_llm_regressor(model, model_name, max_new_tokens=1, temperature=0.0):
    ...
    Returns:
        A function that implements LLM regression with the specified
        configuration
...
def knn_regression_generic(
    x_train, x_test, y_train, y_test, model_name, knn_kwargs
):
    # knn_args = {**knn_kwargs}
        ...
        for weights in ["uniform", "distance"]:
            for p in [0.25, 0.5, 1, 1.5, 2]:
                ...
                knn_fns.append(
                    ...
                    x_test,
                    y_train,
                    y_test: knn_regression_generic(
                        x_train,
                        x_test,
                        y_train,
                        y_test,
                        model_name=f"knn_search_{idx}",
                        knn_kwargs={
...
def lr_with_polynomial_features_regression(
    x_train, x_test, y_train, y_test, random_state=1, **kwargs
):
...
def linear_regression_manual_gd(
    ...
    x_test = (
        x_test.to_numpy() if hasattr(x_test, "to_numpy") else np.array(x_test)
    )
    y_test = (
...
def create_linear_regression_gd_variants(
    ...
    def create_gd_function(
        steps, learning_rate, init_weights, momentum, lr_schedule
    ):
